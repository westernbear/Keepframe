...
def verify(scene: Scene, scene_dir: Path, render_result: RenderResult | None = None,
    ...
    if render_result is not None:
        ...
        for el in scene.elements:
            probes = render_result.bboxes.get(el.id)
            visible_indices = [i for i, f in enumerate(render_result.frames) if el.visible[0] <= f <= el.visible[1]]
            if not visible_indices or probes is None or len(probes) != len(render_result.frames):
                rep.layer_probe_complete = False
                continue
            if any(len(probes[i]) != 4 or not np.isfinite(probes[i]).all() for i in visible_indices):
                rep.layer_probe_complete = False
                continue
            for i, f in enumerate(render_result.frames):
                if not (el.visible[0] <= f <= el.visible[1]):
                    continue
                if len(probes[i]) != 4 or not np.isfinite(probes[i]).all():
                    rep.layer_probe_complete = False
                    continue
                exp = np.array(element_bbox(el, f)); act = np.array(probes[i])
    ...
    rep.passed = rep.schema_ok and rep.keep_pass_rate == 1.0 and rep.layer_probe_complete and rep.layer_max_err_px <= tol_px
    return rep
