...
def _ecc_constants(canon: np.ndarray, anchor: tuple[float, float]) -> tuple[np.ndarray, np.ndarray]:
    """Anchor-to-center matrix and premultiplied grayscale template — canon constants, hoisted per track."""
...
def sprite_props(track: ObjectTrack, frames: np.ndarray, bg_rgb: tuple, n_frames: int, first_frame: int,
    ...
    # Canon-derived constants: computed once per track instead of once per frame.
